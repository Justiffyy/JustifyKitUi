...
<template>
    <div class="min-h-screen flex flex-col bg-slate-50">
        ...
        <Navbar v-if="showNavbar" :title="title">
            <template #left>
                <button class="lg:hidden p-2 rounded-lg text-slate-500 hover:bg-slate-100" @click="toggleMobile">
        ...
        <div class="flex flex-1">
            ...
            <!-- Sidebar -->
            <aside :class="[
                'fixed lg:static inset-y-0 z-50 bg-white border-r border-slate-200 flex flex-col transition-all duration-300',
                ...
                <div class="flex items-center justify-between p-4 border-b border-slate-100">
                    <span v-if="!isCollapsed" class="font-semibold text-slate-900">Menu</span>
                    <button v-if="collapsible" @click="toggle"
                        class="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 hidden lg:block">
                    ...
                    <button class="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 lg:hidden" @click="closeMobile">
